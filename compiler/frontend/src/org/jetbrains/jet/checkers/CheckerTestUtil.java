...
public class CheckerTestUtil {
    ...
    public static List<Diagnostic> getDiagnosticsIncludingSyntaxErrors(BindingContext bindingContext, final PsiElement root) {
        ...
        ImmutableMap<JetExpression,JetType> autoCastedExpressions = bindingContext.getSliceContents(BindingContext.AUTOCAST);
        for (JetExpression expression : autoCastedExpressions.keySet()) {
            diagnostics.add(new DebugInfoDiagnostic(expression, DebugInfoDiagnosticFactory.AUTOCAST));
        }
        return diagnostics;
    }
